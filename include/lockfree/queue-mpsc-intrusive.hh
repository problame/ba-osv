...
#ifndef INCLUDED_LOCKFREE_QUEUE_MPSC_INTRUSIVE
#define INCLUDED_LOCKFREE_QUEUE_MPSC_INTRUSIVE
...
// LT can be any type that has an "LT_next_member" field, which we used to hold a
// pointer to the next item in the queue.
template <typename LT, LT* LT::* LT_next_member>
class queue_mpsc_intrusive {
private:
    std::atomic<LT*> pushlist;
    LT* poplist;
public:
    constexpr queue_mpsc_intrusive<LT, LT_next_member>() : pushlist(nullptr), poplist(nullptr) { }
    ...
    inline void push(LT* item)
    {
        ...
        // item->*LT_next_member, and changing the head.
    ...
    inline LT* pop()
    {
        if (poplist) {
            ...
            LT* result = poplist;
            poplist = poplist->*LT_next_member;
            return result;
        } else {
            ...
            // of the pop.
            while (r->*LT_next_member) {
                LT *next = r->*LT_next_member;
                r->*LT_next_member = poplist;
                poplist = r;
                r = next;
            }
            return r;
